...
pub struct OrderEntryPool {
    // Replace UnsafeCell with a thread-safe alternative like RwLock
...
pub struct Orderbook {
    ...
    bids: RwLock<BTreeMap<NotNan<f64>, VecDeque<u64>>>, // Price -> vector of indices into the memory pool
    asks: RwLock<BTreeMap<NotNan<f64>, VecDeque<u64>>>,
...
impl Orderbook {
    ...
    pub fn update(&self) -> Result<Duration, &'static str> {
        ...
        // Acquire read locks for the orderbook data
        let bids = match self.bids.read() {
            Ok(bids) => bids,
            Err(_) => return Err("Failed to acquire bids read lock"),
        };
        
        let asks = match self.asks.read() {
            Ok(asks) => asks,
            Err(_) => return Err("Failed to acquire asks read lock"),
        };
        
        // Calculate all metrics
        self.calculate_depths(&bids, &asks, &mut metrics);
        self.calculate_mid_price(&mut metrics);
        self.calculate_spread(&mut metrics);
        self.calculate_drift(&mut metrics);
        self.calculate_variance(&bids, &asks, &mut metrics);
        self.calculate_depth_by_tick(&bids, &asks, &mut metrics);
    ...
    #[inline]
    fn best_bid(&self, bids: &BTreeMap<NotNan<f64>, VecDeque<u64>>) -> Option<f64> {
        bids.keys().rev().next().map(|p| p.into_inner())
    }

    #[inline]
    fn best_ask(&self, asks: &BTreeMap<NotNan<f64>, VecDeque<u64>>) -> Option<f64> {
        asks.keys().next().map(|p| p.into_inner())
    }
    ...
    fn calculate_depths(
        &self, 
        bids: &BTreeMap<NotNan<f64>, VecDeque<u64>>, 
        asks: &BTreeMap<NotNan<f64>, VecDeque<u64>>,
        ...
        metrics.total_bid_depth = bids.iter()
            .fold(0.0, |acc, (_, indices)| {
                acc + indices.iter().fold(0.0, |sum, &idx| {
    ...
    fn calculate_depth_by_tick(
        ...
        for (price, indices) in bids {
            let total_depth = indices.iter().fold(0.0, |sum, &idx| {
                sum + self.entry_pool.get(idx).map_or(0.0, |entry| entry.quantity)
            });
...
impl Orderbook {
    ...
    #[inline]
    pub fn add_limit_bid(&self, price: f64, order_id: u64, quantity: f64, timestamp: u64) -> Result<u64, &'static str> {
        // Validate inputs
        if quantity <= 0.0 {
        ...
        // Acquire write lock for bids
        let mut bids = match self.bids.write() {
            Ok(bids) => bids,
            Err(_) => {
                self.entry_pool.free(order_idx);
                return Err("Failed to acquire bids write lock");
            }
        };
        
        // Get or create price level
        let price_level = bids.entry(ordered_price).or_insert_with(VecDeque::new);
        
        // Add order index to price level
        price_level.push_back(order_idx);
    ...
    #[inline]
    pub fn remove_limit_bid(&self, price: f64, order_id: u64) -> Result<bool, &'static str> {
        // Convert price to NotNan for the BTreeMap key
        let ordered_price = match NotNan::new(price) {
            Ok(p) => p,
            Err(_) => return Err("Invalid price (NaN)"),
        ...
        let mut bids = match self.bids.write() {
            Ok(bids) => bids,
            Err(_) => return Err("Failed to acquire bids write lock"),
        };
        
        if let Some(price_level) = bids.get_mut(&ordered_price) {
            let len_before = price_level.len();
            
            // Find the order index and remove it
            let mut i = 0;
            while i < price_level.len() {
                // First, check if the order ID matches without removing yet
                let mut should_remove = false;
                let idx = price_level[i];
                ...
                if should_remove {
                    ...
                    let removed_idx = price_level.remove(i).expect("Failed to remove order index");
                    self.entry_pool.free(removed_idx);
                    break;
                } else {
                    i += 1;
                }
            ...
            // Check if we need to remove the price level
            let should_remove_price_level = price_level.is_empty();
            
            // Return true if an order was removed
            let order_removed = price_level.len() < len_before;
            
            // We need to check outside of the if block to avoid the double borrow
            if should_remove_price_level {
                bids.remove(&ordered_price);
            }
            
            return Ok(order_removed);
        }
    ...
    #[inline]
    pub fn edit_limit_bid(&self, price: f64, order_id: u64, new_quantity: f64) -> Result<bool, &'static str> {
        if new_quantity <= 0.0 {
            return Err("Quantity must be positive");
        }
        ...
        if let Some(price_level) = bids.get(&ordered_price) {
            // Find the order and update quantity
            for &idx in price_level {
                ...
                // The get_mut method now returns a boolean indicating if the update was applied
                if updated {
                    return Ok(true);
                }
            }
    ...
    #[inline]
    pub fn modify_limit_bid(&self, old_price: f64, order_id: u64, new_price: f64, new_quantity: f64, timestamp: u64) 
    -> Result<bool, &'static str> 
    {
        ...
        // Use a write lock for the whole operation to ensure atomicity
        let mut bids = match self.bids.write() {
            Ok(bids) => bids,
            Err(_) => return Err("Failed to acquire bids write lock"),
        };
        
        // First, find and remove the order from the old price level
        let mut found = false;
        let mut removed_idx = None;
        
        if let Some(price_level) = bids.get_mut(&old_ordered_price) {
            let mut i = 0;
            while i < price_level.len() {
                // Get current entry at the index
                let idx = price_level[i];
                let entry_opt = self.entry_pool.get(idx);
                
                // Check if it's our target order
                let should_remove = match &entry_opt {
                    Some(entry) => entry.order_id == order_id,
                    None => false
                };
                
                if should_remove {
                    found = true;
                    removed_idx = Some(price_level.remove(i));
                    break;
                }
                i += 1;
            }
            ...
            let should_remove_price_level = price_level.is_empty();
            if should_remove_price_level {
                bids.remove(&old_ordered_price);
            }
        }
        
        // If order was found, update it and move to new price
        if let Some(idx) = removed_idx {
            let idx = idx.expect("Failed to remove order index");
            // Update the existing entry with closure
            let updated = self.entry_pool.get_mut(idx, |entry| {
                entry.quantity = new_quantity;
            ...
            if updated {
                ...
                let price_level = bids.entry(new_ordered_price).or_insert_with(VecDeque::new);
                price_level.push_back(idx);
            ...
            // Order found but already removed
            Ok(false)
        } else {
            // Order not found, create a new one
            drop(bids); // Release the write lock
    ...
    #[inline]
    pub fn add_limit_ask(&self, price: f64, order_id: u64, quantity: f64, timestamp: u64) -> Result<u64, &'static str> {
        // Validate inputs
        if quantity <= 0.0 {
            return Err("Quantity must be positive");
        }
        ...
        // Acquire write lock for asks
        let mut asks = match self.asks.write() {
            Ok(asks) => asks,
            Err(_) => {
                self.entry_pool.free(order_idx);
                return Err("Failed to acquire asks write lock");
            }
        };
        
        // Get or create price level
        let price_level = asks.entry(ordered_price).or_insert_with(VecDeque::new);
    ...
    // Method to match a market bid (buy) order
    #[inline]
    pub fn match_market_bid(&self, price_limit: f64, quantity: f64) 
    -> Result<ExecutionResult, &'static str> 
    {
        ...
        let mut asks = match self.asks.write() {
            Ok(asks) => asks,
            Err(_) => return Err("Failed to acquire asks write lock"),
        };
        
        // Get all eligible ask price levels (price <= price_limit)
        let eligible_prices: Vec<NotNan<f64>> = asks.keys()
        ...
        for ask_price in eligible_prices {
            ...
            if let Some(ask_orders) = asks.get_mut(&ask_price) {
                ...
                for idx in orders_to_remove.iter().rev() {
                    ...
                    if let Some(removed_idx) = ask_orders.remove(*idx) {
                        self.entry_pool.free(removed_idx);
                    }
                }
    ...
    #[inline]
    pub fn remove_limit_ask(&self, price: f64, order_id: u64) -> Result<bool, &'static str> {
        // Convert price to NotNan for the BTreeMap key
        let ordered_price = match NotNan::new(price) {
            Ok(p) => p,
            Err(_) => return Err("Invalid price (NaN)"),
        ...
        if let Some(price_level) = asks.get_mut(&ordered_price) {
            let len_before = price_level.len();
    ...
    #[inline]
    pub fn edit_limit_ask(&self, price: f64, order_id: u64, new_quantity: f64) -> Result<bool, &'static str> {
        if new_quantity <= 0.0 {
            return Err("Quantity must be positive");
        }
        ...
        if let Some(price_level) = asks.get(&ordered_price) {
            // Find the order and update quantity
            for &idx in price_level {
                // Using the new get_mut method with a closure
                let updated = self.entry_pool.get_mut(idx, |entry| {
    ...
    #[inline]
    pub fn modify_limit_ask(&self, old_price: f64, order_id: u64, new_price: f64, new_quantity: f64, timestamp: u64) 
    -> Result<bool, &'static str> 
    {
        // First, find and remove the existing order
        let old_ordered_price = match NotNan::new(old_price) {
        ...
        if let Some(price_level) = asks.get_mut(&old_ordered_price) {
            let mut i = 0;
            while i < price_level.len() {
                // Get current entry at the index
                let idx = price_level[i];
        ...
        if let Some(idx) = removed_idx {
            ...
            if updated {
                // Add to the new price level
                let price_level = asks.entry(new_ordered_price).or_insert_with(VecDeque::new);
                price_level.push_back(idx);
            ...
            // Order not found, create a new one
            drop(asks); // Release the write lock
    ...
    #[inline]
    pub fn match_market_ask(&self, price_limit: f64, quantity: f64) 
    -> Result<ExecutionResult, &'static str> 
    {
        let start = Instant::now();
        ...
        // Must collect into a vector to get highest to lowest
        let eligible_prices: Vec<NotNan<f64>> = bids.keys()
        ...
        for bid_price in eligible_prices {
            ...
            if let Some(bid_orders) = bids.get_mut(&bid_price) {
                ...
                for idx in orders_to_remove.iter().rev() {
                    // Free the memory pool entry
                    if let Some(removed_idx) = bid_orders.remove(*idx) {
                        self.entry_pool.free(removed_idx);
                    }
                }
    ...
    // Get current orderbook state as vectors for visualization
    pub fn get_orderbook_levels(&self, depth: usize) -> Result<(Vec<(f64, f64)>, Vec<(f64, f64)>), &'static str> {
        ...
        for (price, indices) in bids.iter().rev().take(depth) {
            let total_quantity = indices.iter().fold(0.0, |sum, &idx| {
                sum + self.entry_pool.get(idx).map_or(0.0, |entry| entry.quantity)
            });
        ...
        for (price, indices) in asks.iter().take(depth) {
            let total_quantity = indices.iter().fold(0.0, |sum, &idx| {
                sum + self.entry_pool.get(idx).map_or(0.0, |entry| entry.quantity)
            });
...
mod tests {
    ...
    fn test_concurrent_access() {
        let orderbook = Arc::new(Orderbook::new("BTC/USD".to_string(), "Kraken".to_string(), 100000));
        ...
        for i in 0..5 {
            ...
            let handle = thread::spawn(move || {
                for j in 0..100 {
                    ...
                    ob.add_limit_bid(price, order_id, 1.0, order_id).unwrap();
                }
            });
            handles.push(handle);
        }
        ...
        for i in 5..10 {
            ...
            let handle = thread::spawn(move || {
                for j in 0..100 {
                    ...
                    ob.add_limit_ask(price, order_id, 1.0, order_id).unwrap();
                }
            });
            handles.push(handle);
        }
    ...
    fn test_performance_benchmarks() {
        let orderbook = Orderbook::new("BTC/USD".to_string(), "Kraken".to_string(), 100000);
    ...
    fn test_memory_pool_capacity() {
        ...
        let orderbook = Orderbook::new("BTC/USD".to_string(), "Kraken".to_string(), 5);
    ...
    fn test_memory_cleanup() {
        ...
        // The memory pool doesn't reuse freed entries in this implementation,
        // so we've exhausted the allocation
    ...
    fn test_error_propagation() {
        ...
        let orderbook = Orderbook::new("BTC/USD".to_string(), "Kraken".to_string(), 1);
    ...
    fn test_large_number_of_price_levels() {
        ...
        // Verify metrics
        let metrics = orderbook.metrics().unwrap();
        assert_eq!(metrics.best_bid, 99.99); // Highest bid
        assert_eq!(metrics.best_ask, 100.01); // Lowest ask
