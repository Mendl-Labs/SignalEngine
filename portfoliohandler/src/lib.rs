...
use signalengine::{SignalEngineLogger, TradingContext, log_info_async, log_warn_async, log_error_async};
...
impl PortfolioHandler {
    ...
    async fn log_portfolio_stats(&self) -> Result<(), Box<dyn Error>> {        
    ...
    fn listen(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        loop {
            ...
            if last_stats_time.elapsed() >= stats_interval {
                ...
                let logger = self.logger.clone();
                let portfolio_task = self.log_portfolio_stats();
                tokio::spawn(async move {
                    if let Err(e) = portfolio_task.await {
                        logger.error(&format!("Error logging portfolio stats: {}", e)).await;
                    }
                });
