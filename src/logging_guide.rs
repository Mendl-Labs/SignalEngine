...
mod examples {
    ...
    impl ExampleTradingComponent {
        pub async fn new() -> Self {
            let logger = Arc::new(SignalEngineLogger::new("ExampleComponent").await);
        ...
        pub async fn execute_trade(&self, symbol: &str, quantity: f64, price: f64) -> Result<String, String> {
            ...
            let order_id = format!("order_{}", fastrand::u64(..));
        ...
        pub async fn generate_signal(&self, symbol: &str, action: &str) -> u64 {
            let signal_id = fastrand::u64(..);
            
            // Use high-performance macro for signal logging
            log_trading_signal!(self.logger, signal_id, symbol, action, "NORMAL");
    ...
    async fn test_comprehensive_logging() {
        initialize_signal_engine().await.unwrap();
