...
impl SignalEngineLogger {
    ...
    /// Critical/Fatal level logging
    pub async fn critical(&self, message: &str) {
        self.log_with_context(LogLevel::Fatal, message, None).await;
    }
    
    /// Critical with trading context
    pub async fn critical_ctx(&self, message: &str, context: TradingContext) {
        self.log_with_context(LogLevel::Fatal, message, Some(context)).await;
    }
