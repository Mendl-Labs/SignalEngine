...
pub struct KrakenConnector {
    ...
    order_updates: Arc<SPSCQueue<OrderUpdate>>,
...
impl KrakenConnector {
    pub fn new() -> Self {
        Self {
            ...
            order_updates: Arc::new(SPSCQueue::<OrderUpdate>::new()),
...
impl ExchangeConnector for KrakenConnector {
    ...
    async fn cancel_order(&self, order_id: &str) -> Result<CancelResult, ExecutionError> {
        ...
        let cancel_latency = timer.elapsed_ns();
    ...
    fn get_metrics(&self) -> ExecutionMetrics {
        ...
        ExecutionMetrics {
            ...
            websocket_connected: self.websocket.as_ref().map_or(false, |ws| ws.is_connected()),
